...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["556193845817@s.whatsapp.net"] 
			...
			switch(command) {
				...
				case 'info':
					...
					break
				case 'listabloq': 
					teks = 'Burrin bloquado com sucesso :\n'
...
case 'randomhentai':
:gataudabody.slice(6)
 reply(mess.wait)  anu = await fetchJson(`https://tobz-api.herokuapp.com/api/hentai?apikey=BotWeA`, {method: 'get'})
    buffer = await getBuffer(anu.result)
  client.sendMessage(from, buffer, image, {quoted: mek})
          break
				case 'ytmp4':
			...
			case 'mapa': 
				...
				case 'tts':
				case 'audio':
					if (args.length < 1) return client.sendMessage(from, 'idioma e stickernescessario!!', text, {quoted: mek})
